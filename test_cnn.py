...
def test_model(imgpath, maskpath, labelpath, project_home_dir, min_epoch):
    """Test CNN model on test data. Analyze model outputs and output csv file with predictions.
    Keyword arguments:
    ...
    model_path = project_home_dir + 'model/' + config['model_dir'] + '/'
    
    print("pretrained dir:", config['model_dir'])
    pretrained_dir = config['model_dir']
    ...
    model_setup.setup_model(tmodel, "test", config['cvphase'], config['model_dir'], pretrained_dir, project_home_dir, min_epoch)
