...
def test_transformer(saved_model_dir, project_home_dir, min_epoch):
    """Test Transformer model on test features. Analyze model outputs and output csv file with predictions.
    Keyword arguments:
    saved_model_dir -- model path to load pretrained weights from (feed into setup_model function).
    min_epoch -- lowest validation loss epoch to use for pretrained weights.
    """
