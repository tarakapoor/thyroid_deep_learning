...
def train_model(imgpath, maskpath, labelpath, project_home_dir):
    """Train CNN model on train data and validate on val data.
    ...
    Return lowest validation loss epoch.
    """
    config = configread()
    ...
    if(config['trainval'] == True):
        config = configwrite('num_epochs', best_epoch+2)
    #else:
    #    config = configwrite('num_epochs', 100)
    ...
    if(not config['trainval']):
        model_dir = config['modeltype'] + '_thyroid_weighted_focal_extralinear_adj_trainonly'
        model_path = project_home_dir + 'model/' + model_dir + '/'
    else: #trainval
        model_dir = config['modeltype'] + '_thyroid_weighted_focal_extralinear_adj'
        model_path = project_home_dir + 'model/' + model_dir + '/'
        
    config = configwrite('model_dir', model_dir)
    pretrained_dir = model_dir #only used by setup_model if in test phase
    
    config = configwrite('phase', "train")                           
    model_setup.setup_model(model, "train", config['cvphase'], model_dir, pretrained_dir, project_home_dir, config['best_epoch'])
    ...
    #load DATA
    if(not config['trainval']):
        train_set = mobilenet_dataset.DatasetThyroid3StackedNew(imgpath, maskpath, labelpath, project_home_dir, "train", config['cvphase'], config['frametype'], transform=transformAug)
        trainlen = len(train_set)
    else:
        trainval_set = mobilenet_dataset.DatasetThyroid3StackedNew(imgpath, maskpath, labelpath, project_home_dir, "trainval", config['cvphase'], config['frametype'], transform=transformAug)
        trainlen = len(trainval_set)
    val_set = mobilenet_dataset.DatasetThyroid3StackedNew(imgpath, maskpath, labelpath, project_home_dir, "val", config['cvphase'], config['frametype'], transform=transformNorm)
    
    print("\n\nbefore epochs: len datasets", trainlen, len(val_set))
        
    config = configread() #incase there were updates
    ...
    for epoch in range(config['num_epochs']):
        ...
        f_losses.append(running_focal_loss / traincount)
    ...
    analyze_model_outputs.plot_test_stats(losses, f_losses, losses_val, f_losses_val, epoch_aurocs, patientlabels, patient_ave_preds, "mobilenet")
