...
def plot_test_stats(losses, f_losses, losses_val, f_losses_val, epoch_aurocs, patientlabels, patient_ave_preds):
    ...
    #########
    if (not args.trainval): #thresholds
    ...
    print("thresholds:", args.best_thresh, args.best_thresh_2fpr)
...
def calc_test_stats(distinct_patients, patientlabels, patient_ave_preds):
    ...
    for pp in range(len(patient_ave_preds)):
        if (patient_ave_preds[pp] >= args.best_thresh):
    ...
    print("threshold probability:", args.best_thresh)
    ...
    print('done, best epoch:', args.best_epoch)
...
def analyze_test_outputs(savefilename, modeltype): #modeltype cnn or transformer
    ...
    test_all_probs_ones = []
    rowcount = 0
    testsaveallfile = savefilename#"test_all_outs" + str(args.cvphase) + ".csv"
    ...
    print('thresh %.8f\nthresh fpr weighted %.5f\nthresh tpr weighted %.8f' % (args.best_thresh, args.best_thresh_2fpr, args.best_thresh_2tpr))
    thresh = args.best_thresh_2fpr #using false positive threshold now for confusion matrix
    ...
    testsavefile = "saved_test_" + str(modeltype) + str(args.cvphase) + "_adj.csv"
