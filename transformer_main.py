...
def transfconfigwrite(varname, value): #for yaml
    """Write new value for variable to config yaml file for Transformer.
...
def transfconfigread():
    """Read config yaml file for Transformer.
...
def main():
    """Main function for extracting features from CNN, then using them for training and testing Transformer model and saving outputs.
    Transformer config file is updated automatically within functions in other files."""
    ...
    bestepoch, saved_model_dir = train_transformer.train_transformer(args.project_home_dir)
    ...
    realtrain, saved_model_dir = train_transformer.train_transformer(args.project_home_dir) #ignore this returned epoch
    ...
    test_transformer.test_transformer(saved_model_dir, args.project_home_dir, transfconfig['best_epoch'])
