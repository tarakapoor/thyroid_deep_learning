...
def load_csv_padded_transformer(phase, cvphase, frametype, project_home_dir): #no padded 2d features    
    ...
    return(allfeatures, alllabels, allids, allframenums, imgsperpat) #return feature vectors and labels and patient ids for transformer
...
class DatasetPaddedTransformer(data.Dataset):
    def __init__(self, phase, cvphase, concat_features, vertconcat, frametype, project_home_dir):
        ...
        if(concat_features): #horizontal concat or vertical concatenated 2d features
            if(vertconcat):
                self.all_frames, self.all_labels, self.all_annot_ids, self.all_frame_nums, self.imgsperpat = load_csv_padded_transformer_vertical_concat(phase, cvphase, frametype, project_home_dir, features2dpath)
            else:
                self.all_frames, self.all_labels, self.all_annot_ids, self.all_frame_nums, self.imgsperpat = load_csv_padded_transformer_horiz_concat(phase, cvphase, frametype, project_home_dir, features2dpath)
        else: #no concatenated 2d features
            self.all_frames, self.all_labels, self.all_annot_ids, self.all_frame_nums, self.imgsperpat = load_csv_padded_transformer(phase, cvphase, frametype, project_home_dir)
    ...
    def __getitem__(self, i):
        ...
        #if(self.phase == "test"):
            #print("index:", i, "of start indices:", len(self.start_indices), "and starting index", startind, "= patient", self.all_annot_ids[startind])
        try:
